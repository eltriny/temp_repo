...
# 로깅 설정 — 프로젝트 네임스페이스 로거에 직접 핸들러 설정
# PaddlePaddle C++ 엔진(glog)이 root logger를 오염시키는 문제 우회
# (paddle/base/log_helper.py 주석 참고: "setting basicConfig in paddle
...
_LOG_DIR = Path(os.environ.get("VIDEO_ANALYZER_LOG_DIR", "."))
_LOG_DIR.mkdir(parents=True, exist_ok=True)
_log_file_handler = RotatingFileHandler(
    _LOG_DIR / "video_analyzer.log",
...
# root logger 설정 (외부 라이브러리 호환성 유지)
logging.basicConfig(level=logging.INFO, format=_LOG_FORMAT)

# 프로젝트 네임스페이스 로거 — root logger와 독립 동작
# src.* 하위 모든 로거가 이 설정을 상속받아 PaddleOCR 간섭에 영향받지 않음
_project_logger = logging.getLogger("src")
_project_logger.setLevel(logging.INFO)
_project_logger.addHandler(_log_stream_handler)
_project_logger.addHandler(_log_file_handler)
...
사용 예시:
  ...
  # 배치 모드 - 1회 스캔/처리 후 종료 (Windows Task Scheduler에서 주기적 호출)
  python main.py --watch-dir ./videos --template "모니터링 레이아웃 A"
  python main.py --watch-dir ./videos --template-id 1
        """,
    )
    ...
    batch_group.add_argument(
        ...
        type=Path,
        metavar="DIR",
        help="배치 스캔 디렉토리. 1회 스캔/처리 사이클 실행 후 종료. "
        "Windows Task Scheduler에서 주기적으로 호출하세요.",
    )
...
def main():
    ...
    # 디버그 모드 — 프로젝트 로거에 적용 (root logger 대신)
    if args.debug:
    ...
    # 배치 모드: --watch-dir 지정 시 단일 사이클 실행
    if args.watch_dir:
        ...
        exit_code = scheduler.run_single_cycle()
        sys.exit(exit_code)
